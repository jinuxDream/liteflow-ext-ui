import { Util, Graph, Rectangle } from '@antv/x6';
...
export function findViewsFromPoint(flowGraph: Graph, x: number, y: number) {
  return flowGraph
    ...
    .filter((view) => {
      if (view !== null) {
        let bBox = Util.getBBox(view.container as any, {
...
const registerEvents = (flowGraph: Graph, showEdgeAddButton: boolean = true): void => {
  ...
  if (!showEdgeAddButton) {
    flowGraph.on('edge:added', (args) => {
      args.edge.setLabel('');
    });
    
    flowGraph.on('edge:connected', (args) => {
      args.edge?.setLabel('');
    });
  }

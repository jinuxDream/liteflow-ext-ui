...
const FlowGraphContextMenu: React.FC<IProps> = (props) => {
  ...
  const { x, y, scene, visible, flowGraph, enableEdit = true } = props;
  ...
  const makeMenuContent = useCallback((flowGraph: Graph, menuConfig: IMenuConfig[]) => {
    const loop = (config: IMenuConfig[]) => {
      return config.map((item) => {
        ...
        if (!enableEdit && (key === 'delete' || key === 'paste')) {
          disabled = true;
        }
  ...
  return !visible ? null : (
    ...
    >
      <Menu mode={'vertical'} selectable={false} onClick={onClickMenu}>
        {makeMenuContent(flowGraph, menuConfig)}
      </Menu>
    </div>
  );
};
